...
def main():
    ...
    OUT_INDICE.write_text(json.dumps(
        [{"nombre": e["nombre"], "apellido": e["apellido"], "curso": e["curso"],
          ...
          "rank": e["rank"], "estimado_s1": e["estimado"]} for e in ests],
...
.aviso{background:#fff1f0;border:1px solid #ffa39e;border-left:5px solid #cf1322;border-radius:6px;
  padding:13px 17px;font-size:13px;color:#5c0011;line-height:1.55;margin-bottom:16px;}
...
def render_reporte(e, total, n_tot):
    ...
    aviso = ""
    if e["estimado"]:
        aviso = ('<div class="aviso"><b>Aviso sobre este informe.</b> Este estudiante no aparece en el '
                 'registro de respuestas de la <b>Sesión 1</b> (Matemáticas 1–22, Lectura Crítica 23–58, '
                 'Sociales 59–80 y Ciencias Naturales 81–106). Los puntajes de esa sesión —incluida la '
                 'totalidad de <b>Lectura Crítica</b>— son una <b>estimación estadística</b> calculada a '
                 'partir de su desempeño real en la Sesión 2. Los resultados de la Sesión 2 (Sociales, '
                 'Matemáticas, Ciencias Naturales e Inglés) sí son reales. Tómese el puntaje global como '
                 'orientativo y no como una calificación definitiva.</div>')
...
def escribir_buscador(ests):
    datos = json.dumps([{"n": e["nombre"], "c": e["curso"], "g": e["icfes"],
                         "r": e["rank"], "f": e["archivo"], "e": int(e["estimado"]),
